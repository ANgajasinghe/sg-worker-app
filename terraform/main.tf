terraform {
  ...
  # Save terraform stats to azure storage account
  # Should create storage before run this
  # az group create -n <tamopstfstates> -l eastus2 - Create Resource Group
  # az storage account create -n <agtamopstf> -g tamopstfstates -l eastus2 --sku Standard_LRS - Create Storage Account
  # az storage container create -n <tfstatedevops> - Create Storage Account Container
  
  backend "azurerm" {
    resource_group_name  = "tamopstfstates"
    storage_account_name = "agtamopstf"
    container_name       = "tfstatedevops"
    key                  = "sg-customer-prod-status.tfstate"
  }
}
...
# Create azure function app plan
